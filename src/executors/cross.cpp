...
bool semanticParseCROSS()
{
    ...
    if(matrixCatalogue.isMatrix(parsedQuery.crossResultRelationName)){
        cout << "SEMANTIC ERROR: Resultant relation name clashes with existing matrix name" << endl;
        return false;
    }
...
void executeCROSS()
{
    ...
    while (!row1.empty())
    {
        ...
        while (!row2.empty())
        {
            ...
            resultantTable->writeRow<int>(resultantRow); //putting in csv format, stored in temp source file
            row2 = cursor2.getNext();
        }
